...
export const SmartContractAbi = ({ callbackRoute }: WidgetProps) => {
  const { hasPendingTransactions } = useGetPendingTransactions();
  const getTimeToPong = useGetTimeToPong();
  const {
    // sendPingTransactionFromAbi,
    // sendPongTransactionFromAbi,
    sendUnstakeTransaction, sendstakeTransaction, claimRewards,
    // sendUnstakeTransactionFromAbi, sendUUnstakeTransactionFromAbi,
    transactionStatus
  ...
  const onSendUnstakeTransaction = async () => {
    // await sendPingTransactionFromAbi({ amount: pingAmount, callbackRoute });
    await sendUnstakeTransaction({ callbackRoute });
  };

  const onSendstakeTransaction = async () => {
    await sendstakeTransaction({ callbackRoute });
  };
  const onSendClaim = async () => {
    await claimRewards({ callbackRoute });
  };
      ...
      <div className='flex flex-col gap-2'>
        <div className='flex justify-start gap-2'>
          ...
          <Button
            ...
            onClick={onSendstakeTransaction}
          ...
          <Button
            ...
            onClick={onSendUnstakeTransaction}
